                             ...
                             struct folio **foliop)
{
	...
	for (i=0; i < npages; i++) {
		// returns address to the nth page of inode directory
		kaddr = ext2_get_folio(dir, i, 0, foliop); 
  		if (IS_ERR(kaddr))
        	return ERR_CAST(kaddr);
		
		// get boundary while iterating over pages
		unsigned last_byte = ext2_last_byte(dir, i);  
		de = (ext2_dirent *)kaddr;

		while ((char *)de < (char *)kaddr + last_byte) {
			if (de->inode){
				if (ext2_match(namelen, name, de) == 1)  // MATCH FOUND
					return de; 
			}
			de = ext2_next_entry(de);
		}

		// call folio_release_kmap if ext2_find_entry fails
